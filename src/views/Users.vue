  ...
  <div>
    ...
    <main>
      <div>
        ...
        <article id="main">
          <header>
            ...
            <div>
              ...
              <a v-if="isSuperAdmin(false)" href="" @click.prevent="component = 'AddUser'" class="svg-button">
          ...
          <section>
            <table>
              ...
              <tr v-for="user in users" :key="user['user_id']">
                ...
                <td>
                  <a v-if="isSuperAdmin(false)" href="" @click.prevent="selected_user = user; component = 'EditUser'">
                  ...
                  <a v-if="isSuperAdmin(false)" href="" @click.prevent="selected_user = user; component = 'DeleteUser'">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><!--! Font Awesome Pro 6.1.1 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M135.2 17.69C140.6 6.848 151.7 0 163.8 0H284.2C296.3 0 307.4 6.848 312.8 17.69L320 32H416C433.7 32 448 46.33 448 64C448 81.67 433.7 96 416 96H32C14.33 96 0 81.67 0 64C0 46.33 14.33 32 32 32H128L135.2 17.69zM394.8 466.1C393.2 492.3 372.3 512 346.9 512H101.1C75.75 512 54.77 492.3 53.19 466.1L31.1 128H416L394.8 466.1z"/></svg>
                  </a>
                </td>
              </tr>
        ...
        <article id="buttons">
          <a href="" @click.prevent="checkEmpty('SendEmailUsers')" class="button">{{ $t("send-email") }}</a>
          <a v-if="isSuperAdmin(false)" href="" @click.prevent="checkEmpty('DeleteUsers')" class="button btn-warning">{{ $t("delete-users") }}</a>
        </article>
      </div>
    </main>
  </div>
...
export default {
  ...
  methods: {
    getUsers() {
      API.getUsers(this.email, this.first_name, this.last_name, this.device, this.share)
          .then(response => {
            this.users = response;
          })
          .catch(() => {
    ...
    resetForm() {
      this.email = this.first_name = this.last_name = this.device = this.share = '';
      this.getUsers();
    },
